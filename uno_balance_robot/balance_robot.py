#!/usr/bin/env python3
"""
balance_robot.py (Arduino Uno & Uno Q Bridge Version)

Python script to control an Arduino Uno or Uno Q-based self-balancing robot.
It dynamically detects whether it is running on the Arduino Uno Q's Linux MPU
and uses the internal Bridge library for I2C MPU telemetry. If running on a 
standard PC, it defaults to standard USB Serial telemetry.

Features:
- Spawns local Wi-Fi hosted Web Dashboard at http://localhost:8000.
- Implements PID balancing, Slew limiters, and safety interlocks.
- Implements Stabilization Priority Control (fades driving bias when tilt > 10 deg).
"""
...
<head>
    ...
    <title>Arduino Uno Q Balancing Robot Dashboard</title>
    <style>
        :root {
            --bg-color: #0f172a;
...
<body>
    <header>
        ...
        <div class="subtitle">Arduino Uno Q PID Network Dashboard</div>
    </header>
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/":
            ...
            # Match customized signs that the user is running in their physical robot:
            if direction == "forward":
                web_state["target_offset"] = -3.0  # Lean Forward
                web_state["drive_command"] = -30.0 # Speed bias
                web_state["turn_command"] = 0.0
            elif direction == "backward":
                web_state["target_offset"] = 3.0   # Lean Backward
                web_state["drive_command"] = 30.0  # Speed bias
                web_state["turn_command"] = 0.0
            elif direction == "left":
...
# --- Utility to get local network IP address ---
def get_local_ip():
    try:
...
# --- Unified Control Iteration ---
def run_control_iteration(line_str):
    global last_time, last_dashboard_time, loop_count, last_commanded_speed, right_speed, left_speed
...
# --- Bridge RPC Core Loop for Uno Q Board ---
def bridge_loop():
    global running
    if not running:
        ...
        sys.exit(0)
    try:
        # Call the internal STM32 function via Bridge
        line_str = Bridge.call("get_mpu_data")
        if line_str:
            run_control_iteration(line_str)
    except Exception as e:
        print(f"\n[ERROR] Bridge communication failure: {e}")
        time.sleep(0.01)

# --- Main Program ---
def main():
    global pid, ser_motor, args, running, last_time

    parser = argparse.ArgumentParser(description="Self-Balancing Robot Controller (Arduino Uno / Uno Q)")
    parser.add_argument("--arduino", type=str, default="COM10", help="Arduino serial port (PC only, default: COM10)")
    default_motor = "/dev/ttyACM0" if (USE_BRIDGE or sys.platform.startswith("linux")) else "COM13"
    parser.add_argument("--motor", type=str, default=default_motor, help=f"Motor RS485 serial port (default: {default_motor})")
    parser.add_argument("--baud-arduino", type=int, default=115200, help="Baud rate for Arduino (default: 115200)")
    ...
    # Sync defaults to global web_state
    ...
    print("=" * 60)
    print("      M0601 Self-Balancing Robot Controller (Uno Q/Uno)")
    print("=" * 60)
    if USE_BRIDGE:
        print("Telemetry link: Uno Q Internal Router Bridge")
    else:
        print(f"Telemetry link: USB Serial on {args.arduino}")
    ...
    # Initialize serial port for Motors
    try:
    ...
    # Initialize standard Serial only if running on a PC
    ser_arduino = None
    if not USE_BRIDGE:
        try:
            ser_arduino = serial.Serial(args.arduino, args.baud_arduino, timeout=0.5)
            print(f"[✓] Connected to Arduino USB on {args.arduino}")
        except serial.SerialException as e:
    ...
    def signal_handler(sig, frame):
        global running
        print("\n[!] Shutdown signal detected. Stopping loop...")
        running = False
        if not USE_BRIDGE:
            smooth_stop(ser_motor, right_speed, left_speed)
            try:
    ...
    try:
        ...
        print(f"[✓] Web Dashboard running at:")
    ...
    if USE_BRIDGE:
        # Run loop under Uno Q Bridge environment (App blocks)
        print("[*] Starting Uno Q Bridge interface...")
        App.run(user_loop=bridge_loop)
    else:
        # Run standard PC Serial reading loop
        print("[*] Starting PC serial balancing loop...")
        try:
            while running:
                line = ser_arduino.readline()
                if not line:
                    print("\n[CRITICAL] Arduino Telemetry lost! Timing out...")
                    break
